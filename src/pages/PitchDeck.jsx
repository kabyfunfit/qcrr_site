...
export default function PitchDeck() {
  ...
  }, [currentPage, pages.length, hasStarted]);
  ...
  const handleTouchEnd = (e) => {
    ...
    if (swipeDistance > 50 && currentPage < pages.length - 1) {
      setCurrentPage(p => p + 1); // Swipe Left
    } else if (swipeDistance < -50 && currentPage > 0) {
      setCurrentPage(p => p - 1); // Swipe Right
    }
  };

  // Start Pitch & Request Fullscreen
  const startPitch = () => {
    setHasStarted(true);
    ...
    if (elem.requestFullscreen) {
      ...
      elem.webkitRequestFullscreen().catch((err) => console.log("Fullscreen denied:", err));
    }
  };

  return (
    <>
      {/* MOBILE BLOCKER: Now triggers strictly on portrait orientation, regardless of screen width */}
      <div className="fixed inset-0 z-[100] bg-black text-white flex-col items-center justify-center text-center px-8 portrait:flex landscape:hidden">
        ...
        <p className="text-blue-100/60 font-medium max-w-xs">
          Please rotate your phone to landscape mode to view this pitch deck properly.
        </p>
      </div>

      {/* START SCREEN */}
      {!hasStarted ? (
        <div className="fixed inset-0 z-[90] bg-black flex flex-col items-center justify-center landscape:flex portrait:hidden">
            <img src={Logo} alt="Logo" className="h-24 md:h-32 w-auto mb-12" />
            <button 
              onClick={startPitch}
              className="px-8 py-4 bg-blue-600 text-white font-black uppercase tracking-widest rounded-lg hover:bg-blue-500 transition-colors active:scale-95"
            >
              Start Pitch
            </button>
        </div>
      ) : (
        /* MAIN DECK (Only renders after start is clicked) */
        <div 
          className="relative h-screen w-full bg-black overflow-hidden font-sans select-none flex flex-col landscape:flex portrait:hidden"
        ...
        >
          ...
          <nav className="absolute top-0 left-0 w-full z-50 bg-black/80 backdrop-blur-xl border-b border-blue-900/20 h-16 md:h-20">
            <div className="max-w-7xl mx-auto px-4 md:px-6 h-full flex items-center justify-between relative">
              <div className="absolute left-4 md:left-6 top-0 h-full flex items-start z-[60]">
                <div className="bg-black/40 backdrop-blur-md p-2 md:p-4 rounded-b-3xl border-x border-b border-blue-900/30 shadow-[0_20px_40px_rgba(0,0,0,0.9)]">
                  <img src={Logo} alt="Logo" className="h-10 md:h-24 lg:h-32 w-auto transition-all" />
                </div>
              </div>
              <div className="flex justify-start md:justify-center overflow-x-auto no-scrollbar scroll-smooth flex-grow ml-24 md:ml-0 h-full items-center">
                {pages.map((page, index) => (
                  <button
                    key={index}
                    onClick={() => setCurrentPage(index)}
                    className={`px-3 md:px-6 h-full text-[10px] md:text-xs font-black uppercase tracking-[0.25em] transition-all relative whitespace-nowrap outline-none ${currentPage === index ? 'text-blue-400' : 'text-gray-500 hover:text-white'}`}
                  >
                    {page.title}
          ...
          <div className="flex h-full transition-transform duration-700 cubic-bezier(0.4, 0, 0.2, 1) relative z-10" style={{ transform: `translateX(-${currentPage * 100}vw)`, width: `${pages.length * 100}vw` }}>
            {pages.map((page, index) => (
              <div key={index} className="w-screen h-full flex items-center justify-center px-4 md:px-28 pt-20 md:pt-32 pb-4 md:pb-12 relative min-h-0">
